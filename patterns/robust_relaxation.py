...
def variants(query, policy):
    ...
    if policy['kind'] == 'bounded':
        ...
        low, high = 'min_gap_us','max_gap_us'
    else:
...
def execute(source, query, policy):
    ...
    context = {'profile':PROFILE,'source_sha256':ei.digest(ei.canonical(source)),
               ...
               'artifacts':{p:ei.digest((ei.ROOT/p).read_text()) for p in
                            sorted(set(mixed.FILES + ('patterns/bounded_intervals.py',
    ...
    try:
        snapshot = bt.prepare(source) if policy['kind']=='bounded' else None
        for option in options:
            run = (cohort.execute(snapshot,option['query']) if snapshot else

...
const fields=['fixture','relation','gap-min','gap-max','duration-enabled','duration-min','duration-max','overlap-enabled','overlap-min'];
...
function render(value){
  if(value.result.search_complete!==true)throw new Error('Incomplete query; no completed cohort is available.');
  report=value;
  ...
  $('rows').innerHTML=value.patients.map(row=>`<tr><td>${esc(row.patient_id)}</td><td>${esc(labels[row.status]||row.status)}</td><td><button class="inspect" data-patient="${esc(row.patient_id)}">Inspect</button></td></tr>`).join('');
  $('query').textContent=JSON.stringify(value.query,null,2);

"""One explicit, source-backed pre-index feature for recorded query-by-example.
...
def _feature(detail, profile):
    ...
    for measurement in detail['measurements']:
        ...
        try:
            ...
            if not value.is_finite() or not lower <= timestamp < start:
                continue
...
def references(snapshot):
    profile = feature_profile(snapshot)
    anchors = []
...
def compare(snapshot, reference_token, top_k):
    ...
    if reference['feature'] is None:
        raise ValueError('Reference anchor has no finite same-item/unit pre-index feature')
    ...
    for candidate in eligible:
        ...
        available = [anchor for anchor in anchors if anchor['feature'] is not None]
        if not available:
        ...
        ranked.append({'patient_id': candidate, 'distance': _decimal_string(distance),
                       'unit': options['feature_profile']['unit'], 'selected_anchor': selected,
                       'temporal_status': 'MATCH' if any(row['status'] == 'MATCH' for row in roster
                                                        if row['patient_id'] == candidate)
                                          else 'NO_SELECTED_MATCH'})
    ranked.sort(key=lambda row: (Decimal(row['distance']), row['patient_id'],

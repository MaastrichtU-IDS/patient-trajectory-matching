...
Status: executable opt-in local HTTP service, using the existing authored source fixture and explicit synthetic mapping review. Public clinical mappings remain pending.
...
|---|---:|
| Initial mapped service job and preparation | 1.461 s |
| Changed query through prepared service | 0.027 s |
| Identical changed query from complete-result cache | 0.005 s |
| Fresh mapped session execution for the changed query | 1.365 s |
...
The demo suite now has 67 tests; the contract suite remains 781 checks (758 suite tests and 23 oracle checks). The existing literal pressure route and its public-demo evidence remain separately available. Next steps are a source-configurable mapped route, explicit adoption of clinical reviews, and representative workload measurements under the existing batch-coverage requirements.

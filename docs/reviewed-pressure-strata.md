...
Patient counts are distinct within a stratum. They must not be summed to estimate a combined cohort: patients can occur in more than one stratum, and these aggregate reports do not establish the patient overlap. Calendar declarations count patients with applicable non-empty measurement batches, not matched patients. Measurement counts describe the admitted records selected by the windows, not all observations of that item in the source release.

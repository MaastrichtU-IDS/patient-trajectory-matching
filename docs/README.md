...
6. Run [MIMIC inputevents staging](mimic-inputevents-staging.md) and review its reconciliation and blocked handoff. The [patient-local bridge](patient-local-clocks.md) now supports explicit local bounds. Implement reviewed clinical mappings, endpoint-bound policies and an honest evidence-history contract before connecting staged MIMIC rows to matching.

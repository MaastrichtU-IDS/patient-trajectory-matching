...
The subsequent [clinical source preflight](clinical-source-preflight.md) now supplies the candidate item dossier and a checksum-verified public-demo coverage scan. It found that the original chart table and all-record candidate workload exceed current bounds. Indexed, provenance-preserving window selection is the next technical requirement; the preflight does not execute this mixed query on real-source records.
